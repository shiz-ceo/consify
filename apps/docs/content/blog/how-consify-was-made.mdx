---
title: How consify was made, and why
...
tags: [consify, architecture, fumadocs, react-router, story]
authors: [shiz-ceo]
cover: /blog/how-consify-was-made.svg
coverAlt: A window with lines of code over a purple grid
---

<Authors ids={["shiz-ceo"]} />

This is the first version of consify, so it is a good moment to write down where it came from. It is a long post. It covers the problem, the rule that shaped everything else, the decisions that changed along the way, and what is still missing.

<Callout type="info" title="Status">
  consify is a pre-release. The ideas are settled, the details will still move, and this post will
...
In practice a consify project is a config file, a `content/` folder, an optional `custom/` folder and two files with one line each. There is no `app/` directory, no routes, no layout that you have to keep in sync.
...
The catch is that Fumadocs is a library and not a foundation. It expects you to own an app. consify's job is to be that app once, in a package, and to expose only the decisions that belong to a project.
...
I moved everything to a branch and deleted Next.js completely, so there was no halfway version to maintain. The route modules moved into the package, and `defineRouterConfig(config)` hands the settings to the router. The generated pieces (the content instance, the routes, the root) went into a `.consify/` folder that is ignored by git.
...
- Publishing the package, and a `create-consify` generator.
...
<CTA title="Try consify" description="Your first site takes five minutes." href="/docs/v0/quickstart" label="Read the quickstart" />
