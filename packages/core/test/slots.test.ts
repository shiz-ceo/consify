...
describe("routes of features that are off", () => {
  test("are left out of the list handed to the router", async () => {
    ...
    expect(off).toContain("docs:routes/docs.tsx");
    expect(off).not.toContain("docs:routes/og.ts");
    ...
    expect(on).toContain("blog:routes/blog.tsx");
    expect(on).not.toContain("blog:routes/blog-feed.ts");
    expect(on).toContain("api-reference:routes/api-reference.tsx");
  });
});
